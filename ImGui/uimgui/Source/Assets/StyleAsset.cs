...
namespace UImGui.Assets
{
	...
	internal sealed class StyleAsset : ScriptableObject
	{
		...
		public unsafe void ApplyTo(ImGuiStylePtr s)
		{
			...
			s.WindowPadding = WindowPadding.ToSystemVector();;
			s.WindowRounding = WindowRounding;
			s.WindowBorderSize = WindowBorderSize;
			s.WindowMinSize = WindowMinSize.ToSystemVector();;
			s.WindowTitleAlign = WindowTitleAlign.ToSystemVector();;
			...
			s.FramePadding = FramePadding.ToSystemVector();;
			s.FrameRounding = FrameRounding;
			s.FrameBorderSize = FrameBorderSize;

			s.ItemSpacing = ItemSpacing.ToSystemVector();;
			s.ItemInnerSpacing = ItemInnerSpacing.ToSystemVector();;

			s.CellPadding = CellPadding.ToSystemVector();;

			s.TouchExtraPadding = TouchExtraPadding.ToSystemVector();;
			...
			s.ButtonTextAlign = ButtonTextAlign.ToSystemVector();;

			s.SelectableTextAlign = SelectableTextAlign.ToSystemVector();;

			s.DisplayWindowPadding = DisplayWindowPadding.ToSystemVector();;
			s.DisplaySafeAreaPadding = DisplaySafeAreaPadding.ToSystemVector();;
			...
			{
				s.Colors[colorIndex] = Colors[colorIndex].ToSystemVector();
			}
		}
		...
		public unsafe void SetFrom(ImGuiStylePtr s)
		{
			...
			WindowPadding = s.WindowPadding.ToUnityVector();
			WindowRounding = s.WindowRounding;
			WindowBorderSize = s.WindowBorderSize;
			WindowMinSize = s.WindowMinSize.ToUnityVector();
			WindowTitleAlign = s.WindowTitleAlign.ToUnityVector();
			...
			FramePadding = s.FramePadding.ToUnityVector();
			FrameRounding = s.FrameRounding;
			FrameBorderSize = s.FrameBorderSize;

			ItemSpacing = s.ItemSpacing.ToUnityVector();
			ItemInnerSpacing = s.ItemInnerSpacing.ToUnityVector();

			CellPadding = s.CellPadding.ToUnityVector();

			TouchExtraPadding = s.TouchExtraPadding.ToUnityVector();
			...
			ButtonTextAlign = s.ButtonTextAlign.ToUnityVector();

			SelectableTextAlign = s.SelectableTextAlign.ToUnityVector();

			DisplayWindowPadding = s.DisplayWindowPadding.ToUnityVector();
			DisplaySafeAreaPadding = s.DisplaySafeAreaPadding.ToUnityVector();
			...
			for (int colorIndex = 0; colorIndex < Colors.Length; ++colorIndex)
			{
				Colors[colorIndex] = s.Colors[colorIndex].ToUnityVector();
			}
		}
